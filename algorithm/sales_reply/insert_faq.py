...
def insert_faq(industry_id, brand_id, template_id, faq_content):
    ...
    collection_name = industry_dict[industry_id] + "_" + brand_dict[brand_id] + "_" + template_dict[template_id]

    faq_id = FieldSchema(name='faq_id', dtype=DataType.INT64, is_primary=True)
    ...
    #   vector = m3e_embedding(faq['query'])
    #   faq_record.append({
    #     "faq_id": idx,
    #     "faq_query": vector,
    #     "faq_answer": faq['answer']
    #   })
    # insert_res = milvus_instance.insert(collection_using=collection_identify, data=faq_record)
    ...
    milvus_instance.disconnect_connection(alias_name=alias_name)
    print(f"FAQ insert to {collection_name} collection successfully!")
